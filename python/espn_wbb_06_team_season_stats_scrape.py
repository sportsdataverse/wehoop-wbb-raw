...
response. The downstream R parser in ``wehoop-wbb-data`` reads these
JSONs to build the per-season tidy team-stats frame.

Team ids are sourced from the same place ``scrape_wbb_team_rosters.py``
uses: the season's schedule parquet (``home_id`` / ``away_id``), with a
fallback to ``sdv.wbb.espn_wbb_teams`` if the parquet is missing.

Requirements:
    Depends on the ``espn_wbb_team_stats`` helper added to
    ``sportsdataverse-py`` (sportsdataverse/wbb/wbb_team_stats.py). The
...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_team_stats_logfile.txt",
)
...
def fetch_team_ids_for_season(season: int) -> list[int]:
    ...
    the schedule parquet is missing, then to ``sdv.wbb.espn_wbb_teams``.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WBB team-stats scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of WBB team-stats scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",
