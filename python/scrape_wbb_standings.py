...
"""Scrape ESPN women's-college-basketball standings per season.

Output: ``wbb/standings/json/{season}.json`` -- raw ESPN response. The
downstream R parser in ``wehoop-wbb-data`` reads these JSONs to build
...
Requirements:
    Depends on the ``espn_wbb_standings`` helper added to
    ``sportsdataverse-py`` (sportsdataverse/wbb/wbb_standings.py). The
...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_standings_logfile.txt",
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WBB standings scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of WBB standings scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",
