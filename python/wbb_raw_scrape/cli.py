"""The one CLI contract every WBB raw scraper uses.

``argparse(type=bool)`` is a trap and this repo fell into it: bash hands over
the *string* ``"false"``, and ``bool("false")`` is ``True``. Combined with
``default=True`` it meant both ``-r false`` and omitting the flag re-fetched
every already-captured game, on every daily run, against ESPN.

Parse the text; never cast it. And default to NOT re-scraping: the raw tree is
the checkpoint (see the repo README and the SDV scraping standards).
"""
...
def season_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    Returns:
        ...
        ``start_year``), and ``rescrape: bool`` (defaulting to False).
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Re-fetch payloads already on disk (default: false)",
    )
