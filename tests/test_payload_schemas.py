...
FAMILIES: list[tuple[str, str]] = [
    ("espn_summary", "wbb/json/final/*.json"),
    ("espn_summary", "wbb/json/raw/*.json"),
    ("espn_game_rosters", "wbb/game_rosters/json/*.json"),
    ("espn_officials", "wbb/officials/json/*.json"),
    ("espn_player_core", "wbb/player_core/json/*.json"),
    ("espn_standings", "wbb/standings/json/*.json"),
    ("espn_team_stats", "wbb/team_stats/json/*/*.json"),
    ("espn_player_season_stats", "wbb/player_season_stats/json/*/*.json"),
]
...
def test_raw_and_final_describe_the_same_game_differently():
    ...
    finals = sorted(REPO_ROOT.glob("wbb/json/final/*.json"))
    if not finals:
        pytest.skip("no final captures")
    game_id = finals[0].stem
    raw_path = REPO_ROOT / "wbb" / "json" / "raw" / f"{game_id}.json"
