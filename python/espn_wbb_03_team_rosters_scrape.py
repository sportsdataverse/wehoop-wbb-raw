...
"""Scrape ESPN women's-college-basketball team rosters per (season, team_id).

Output: ``wbb/team_rosters/json/{season}/{team_id}.json`` -- raw ESPN
response. The downstream R parser in ``wehoop-wbb-data`` reads these JSONs
to build the per-season tidy roster frame.

Requirements:
    Depends on the ``espn_wbb_team_roster`` helper added to
    ``sportsdataverse-py`` (sportsdataverse/wbb/wbb_team_roster.py). The
...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_team_rosters_logfile.txt",
)
...
def fetch_team_ids_for_season(season):
    ...
    Sources team ids from this season's schedule parquet (``home_id`` /
    ``away_id``) so we only request rosters for teams that actually played
    that year. Falls back to ``sdv.wbb.espn_wbb_teams`` if the schedule
    parquet is missing.
    """
    schedule_path = Path(f"{PATH_TO_SCHEDULES}/wbb_schedule_{season}.parquet")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WBB roster scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "-e",
        type=int,
        help="End year of WBB roster scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "-c",
