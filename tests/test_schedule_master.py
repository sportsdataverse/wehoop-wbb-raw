...
def _tree(tmp_path: Path) -> Path:
    for sub in (
        ...
        (tmp_path / "wbb" / Path(*sub)).mkdir(parents=True)
    (tmp_path / "wbb" / "json" / "final" / "401811123.json").write_text("{}")
    (tmp_path / "wbb" / "game_rosters" / "json" / "401811123.json").write_text("{}")
    return tmp_path
