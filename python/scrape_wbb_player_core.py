"""Scrape ESPN WBB athlete core records (identity + bio).

Output: ``wbb/player_core/json/{athlete_id}.json`` -- raw ESPN core-v2
...
Athlete ids come from the ``espn_womens_college_basketball_player_boxscores`` release on
...
Requirements:
    Uses ``espn_wbb_player_core`` from sportsdataverse-py.
"""
...
from sportsdataverse.wbb import espn_wbb_player_core
from sportsdataverse.dl_utils import download


logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_player_core_logfile.txt",
)
logger = logging.getLogger(__name__)

PATH_TO_OUTPUT = "wbb/player_core/json"
PLAYER_BOX_RELEASE = (
...
def download_player_core(athlete_id, rerun_existing):
    ...
    try:
        raw = espn_wbb_player_core(athlete_id=int(athlete_id), return_parsed=False)
...
def download_player_core_batch(athlete_ids, rerun_existing, cores):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for fut in tqdm(
            ...
            total=len(futs),
            desc="WBB player core",
        ):
            fut.result()


def main():
    start_year = max(args.start_year, 2004)  # player_box release starts 2004
