...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_officials_logfile.txt",
)
...
def fetch_game_ids_for_season(season: int) -> list[int]:
    """Pull every completed game id ESPN exposes for a given season.
    ...
    fresh ``espn_wbb_schedule`` call when the parquet is missing.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WBB officials scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of WBB officials scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",
