"""One-off: reprocess pre-2006 WBB final.json from the stored raw captures.

Incident: sportsdataverse-py #380 fixed the hard-coded quarters model for
pre-2006 WBB games (2x20-minute halves through 2014-15; wehoop#39 sibling). The scraper bakes the
processed payload into ``wbb/json/final`` at capture time, so every
pre-2006 final.json carries wrong half / seconds-remaining / OT columns.
The raw allowlisted summaries in ``wbb/json/raw`` (which include the
authoritative ``format.regulation.periods``) allow a fully offline rebuild:

    uv run python ops/oneoff/2026-08-23_reprocess_pre2016_halves.py -s 2002 -e 2015
...
RAW_DIR = Path("wbb/json/raw")
FINAL_DIR = Path("wbb/json/final")
SCHED_DIR = Path("wbb/schedules/parquet")
...
def season_game_ids(season: int) -> list[int]:
    df = pd.read_parquet(SCHED_DIR / f"wbb_schedule_{season}.parquet")
...
def main() -> int:
    ...
    ap.add_argument("-e", "--end", type=int, default=2015)
    ...
    for season in range(a.start, a.end + 1):
        ...
        for gid in ids:
            ...
            if not (RAW_DIR / f"{gid}.json").exists():
                ...
                continue
            try:
                pbp_txt = sdv.wbb.wbb_pbp_disk(game_id=gid, path_to_json=str(RAW_DIR))
                result = sdv.wbb.helper_wbb_pbp(game_id=gid, pbp_txt=pbp_txt)
