...
logging.basicConfig(
    level=logging.INFO,
    filename="wehoop_wbb_raw_game_rosters_logfile.txt",
)
...
def download_game_rosters_batch(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for fut in tqdm(
            ...
            total=len(futs),
            desc=f"WBB game rosters {season}",
        ):
            fut.result()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Start year of WBB game-roster scrape (YYYY), e.g. 2025 for the 2024-25 season",
    )
    parser.add_argument(
        "--end_year",
        "--end-year",
        ...
        dest="end_year",
        type=int,
        help="End year of WBB game-roster scrape (YYYY)",
    )
    parser.add_argument(
        "--cores",
        "--workers",
